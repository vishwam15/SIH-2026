...
function BuildRoute({ activeSos, liveLocations }) {
  ...
  useEffect(() => {
    ...
    const fetchRoute = async () => {
      try {
        ...
        console.warn('OSRM route fallback used:', err.message);
      }
      setRoute(directPath);
    };
...
export const LiveGISMap = ({ userRole, currentUserLocation, liveLocations = [], activeSos = [] }) => {
  const defaultCenter = useMemo(() => {
    if (currentUserLocation) return [currentUserLocation.lat, currentUserLocation.lng];
